...
def update_sliders(event=None):
    ...
    if selected_operator and selected_weapon:
        ...
        # Update the label text with the retrieved adjustment values
        update_vertical_label(vertical)
        update_horizontal_label(horizontal)
        update_percentadj_label(percent)
    else:
        # If either the operator or the weapon is not selected, set all sliders to 0
        sldr_vertical.set(0)
...
def update_weapon_box(event=None):
    ...
    if current_weapon not in weapon_values:
        if weapon_values:
            ...
            drop_Weapons.set('')  # Clear the weapon selection if no weapons are available
...
def write_defenders(filename, operators):
    ...
    # Define a function to get the replacement values for a given operator
    def get_replacement_values(operator):
        ...
        for weapon in Operators[operator]["weapons"]:
            values = Operators[operator]["values"][weapon]  # Get the values dictionary for the weapon
            vertical = values["vertical"]
            horizontal = values["horizontal"]
            percent = round(sldr_percentAdjustment.get())
            new_values.append(f"{vertical},{horizontal},{percent}")

        while len(new_values) < 6:  # Ensure there are exactly 6 sets of values
            new_values.append("0,0,80")

        new_values_str = ",".join(new_values[:6])  # Ensure only 6 sets of values are used
        return f'{{{new_values_str}}}'

    del lines[99:119]


    # Insert the new const int8 line at the appropriate position
    const_int8_line = f"const int8 GunAntiRecoil[][] = {{\n"
    for operator in operators:
        const_int8_line += f"\t{get_replacement_values(operator)},\t/* {operator.upper()} */\n"
    const_int8_line += "};\n"
    lines.insert(100, const_int8_line)
...
def write_attackers(filename, operators): # THIS SHIT TOOK ME, 4 HOURS TO FIX, FUCKING KILL ME
    # Read the contents of the source file with UTF-8 encoding
    with open(filename, "r", encoding="utf-8") as file:
        lines = file.readlines()
